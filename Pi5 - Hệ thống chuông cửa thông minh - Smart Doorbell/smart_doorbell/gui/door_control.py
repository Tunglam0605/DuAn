...
def _get_mode():
    mode = os.getenv("SMART_DOORBELL_MODE", "").strip().lower()
    if mode in ("mock", "demo", "desktop"):
        return "mock"
    return "pi"
...
class MockDoorController:
    def __init__(self):
        self.hold_on_face = _env_bool("DOORBELL_DOOR_HOLD_ON_FACE", CONFIG_DOOR_HOLD_ON_FACE)
        self.close_delay_sec = _env_float("DOORBELL_DOOR_CLOSE_DELAY_SEC", CONFIG_DOOR_CLOSE_DELAY_SEC)
        self.require_known = _env_bool("DOORBELL_DOOR_REQUIRE_KNOWN", CONFIG_DOOR_REQUIRE_KNOWN)
        self.require_real = _env_bool("DOORBELL_DOOR_REQUIRE_REAL", CONFIG_DOOR_REQUIRE_REAL)
    ...
    def _update_lcd_state(self, is_open):
        if self._lcd is None:
            return False
        try:
            return bool(self._lcd.set_status(door_open=is_open))
        except Exception:
            return False
    ...
    def handle_result(self, result):
        if not self.hold_on_face or not result:
            return False
        ...
        if self._is_open and now - self._last_seen_ts >= float(self.close_delay_sec):
            self.close()
        return False
